// Binary review verdict contract shared by the review flow, the Electron
// main process, the headless daemon and the web renderer. Pure: no
// Electron, IPC, PTY, zustand or DOM dependencies — safe to import from
// anywhere (moved from src/canvas/reviewVerdict.ts for F2 web-local).
//
 ...
 * A non-array input (bad hydrate, wrong-type persisted payload) degrades to
 * null instead of throwing — render paths call this on every snapshot.
 */
