...
export interface IssueFixPromptInput {
  ...
  // Ruta del repo donde vive `.agents/repo-context.md`: el prompt le dice al
  // agente que lea el archivo por ruta (no se inyecta el contenido).
  repoPath?: string;
}
...
export function buildIssueFixPrompt(input: IssueFixPromptInput): string {
  ...
  return [
    ...
    `DISCIPLINA TDD (skill tdd): el defecto que señaló el reviewer se ataca con un ciclo red-green-refactor NUEVO, no con un parche directo: escribí primero el test que reproduce el bug (en rojo), implementá el fix, y recién cuando esté en verde cerrá el ciclo. Todos los tests del repo deben pasar antes de commitear. `,
    `RESPUESTA AL REVIEW (skill receiving-code-review): ante cada observación del reviewer aplicá rigor técnico y verificación real — corré el caso, mirá el código, probá la alternativa — antes de aceptarla o descartarla. NUNCA respondas con acuerdo performativo ("tenés razón, lo corrijo") sin haber chequeado; si una observación no aplica o es incorrecta, fundamentalo técnicamente en tu réplica. `,
    `PUSH AL PR EXISTENTE: commitá en la rama actual y pusheá con git push origin ${input.branch}. Verificá que el PR se haya actualizado con gh pr view ${input.prNumber} (nuevo encabezado visible). NUNCA crees una rama o PR nuevo, NUNCA abras un PR nuevo y NO mergees el PR. NO cierres el issue manualmente (se cierra solo al mergear via "Closes #${input.issueNumber}"). `,
