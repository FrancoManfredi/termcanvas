...
/**
 * Build a single-line review prompt for the "REVISAR SOLUCIÓN" flow.
 *
...
return [
    ...
    NO_WRAPPER_RULE,
    "REGLAS DE ORO: corré los comandos gh DIRECTO — nunca los envuelvas con rtk ni con wrappers (ocultan stdout y te hacen reintentar a ciegas, duplicando reviews en el PR). Si ves output con prefijo [rtk] o sin output, detenete tras 2 intentos y probá un comando estructuralmente distinto. NUNCA crees una rama nueva, NO hagas commits locales, NO pushees, NO crees un PR nuevo y NO mergees el PR existente. Tu único output es la review en el PR ya existente vía POST /pulls/{pr}/reviews. Analizás en un worktree aislado y descartable: cualquier cambio local que hagas para probar se pierde al cerrar la sesión.",
    `LOGGING PARA DEBUG MANUAL: prefijá cada paso relevante que ejecutes con [review-${input.issueNumber}] para poder filtrar tu trazabilidad en la terminal.`,
  ]
    .filter(Boolean)
    .join(" | ");
}
