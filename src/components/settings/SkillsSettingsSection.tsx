...
export function SkillsSettingsSection() {
  ...
  const [projectList, setProjectList] = useState<Array<{ name: string; path: string }>>([]);
  const [copySource, setCopySource] = useState<string>("");
  const [globalShared, setGlobalShared] = useState<boolean>(true);

  const loadProjects = useCallback(async () => {
    try {
      const maybeProject = (window as unknown as { termcanvas?: { project?: { listChildGitRepos?: (p: string) => Promise<Array<{ name: string; path: string }>> } } }).termcanvas?.project;
      if (maybeProject?.listChildGitRepos) {
        const scriptsDir = (window as unknown as { termcanvas?: { paths?: { scriptsDir?: string } } }).termcanvas?.paths?.scriptsDir ?? "";
        const termcanvasRoot = scriptsDir ? scriptsDir.replace(/[\\/]scripts$/, "") : "C:/Users/Estudiante UCU/OneDrive/Escritorio/termcanvas";
        const parent = termcanvasRoot.replace(/[\\/][^\\/]+$/, "");
        const repos = await maybeProject.listChildGitRepos(parent);
        setProjectList(repos);
        if (!activeProject && repos.length > 0) setActiveProject(repos[0].path);
      }
    } catch {}
  }, [activeProject]);
  ...
  const handlePickFile = async (categoryId: string) => {
    ...
    const res = api.saveFromFileForProject
      ...
      : await api.saveFromFile(categoryId, dlg.filePath, globalShared);
  ...
  const handleFetchSpec = async (categoryId: string) => {
    ...
    const res = api.fetchBySpecForProject
      ...
      : await api.fetchBySpec(spec, categoryId, globalShared);
  ...
  const handlePasteSave = async (categoryId: string) => {
    ...
    const res = api.saveFromContentForProject
      ...
      : await api.saveFromContent(categoryId, name, content, globalShared);
    if (!res.ok) useNotificationStore.getState().notify("error", res.error ?? "Error al guardar");
    else {
  ...
  const handleCopyAll = async () => {
    if (!copySource || !activeProject) {
      useNotificationStore.getState().notify("error", "Elegí un proyecto origen");
      return;
    }
    if (copySource === activeProject) {
  ...
  const handleGlobalShareToggle = async (next: boolean) => {
    setGlobalShared(next);
    if (!activeProject) return;
    const api = (window as unknown as { termcanvas: { skills: { setShareAll?: (repoPath: string, shared: boolean) => Promise<{ ok: boolean; error?: string }> } } }).termcanvas.skills;
    if (api.setShareAll) {
      const res = await api.setShareAll(activeProject, next);
      if (!res.ok) useNotificationStore.getState().notify("error", res.error ?? "No se pudo cambiar");
      else useNotificationStore.getState().notify("info", next ? "Todas compartidas con el equipo" : "Todas privadas");
      await load();
    }
  };
    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-3 rounded-[16px] bg-[var(--surface)]/40 p-4" style={{ boxShadow: "0 1px 2px oklch(0 0 0 / 0.04), 0 4px 12px oklch(0 0 0 / 0.06)", outline: "1px solid oklch(0 0 0 / 0.06)", outlineOffset: "-1px" }}>
        <div className="flex flex-col gap-1.5">
          <span className="text-[11px] font-medium uppercase tracking-[0.08em] text-[var(--text-muted)]">Proyecto activo</span>
          <div className="flex items-center gap-2">
            <select value={activeProject ?? ""} onChange={(e) => setActiveProject(e.target.value)} className="flex-1 rounded-full border border-[var(--border)] bg-[var(--bg)] px-3.5 py-2 text-[12px] text-[var(--text-primary)] outline-none focus:border-[var(--accent)] focus:shadow-[0_0_0_3px_var(--accent-soft)]">
              {projectList.map((p) => (
                <option key={p.path} value={p.path}>{p.name} — {p.path}</option>
              ))}
              {projectList.length === 0 && activeProject && <option value={activeProject}>{activeProject.split(/[\\/]/).pop()} — {activeProject}</option>}
            </select>
            <span className="hidden sm:inline-flex items-center rounded-full bg-[var(--surface)] px-2.5 py-1 text-[11px] font-medium text-[var(--text-secondary)] border border-[var(--border)]">
              {totalSkills} skills
            </span>
          </div>
          <p className="text-[11px] leading-4 text-[var(--text-secondary)]">
            Las skills viven en <span className="font-mono text-[11px] bg-[var(--surface)] px-1 py-0.5 rounded border border-[var(--border)]">&lt;repo&gt;/resources/diagnosis-skills/&lt;categoria&gt;/&lt;skill&gt;/SKILL.md</span> — versionadas con el repo.
          </p>
        </div>
        <div className="flex items-center justify-between gap-3 rounded-[12px] border border-[var(--border)] bg-[var(--bg)]/60 px-3.5 py-3">
          <div className="flex flex-col gap-0.5">
            <span className="text-[12px] font-medium text-[var(--text-primary)]">Compartir con el equipo</span>
            <span className="text-[11px] leading-4 text-[var(--text-secondary)]">Todas las skills de este proyecto — o ninguna.</span>
          </div>
          <button type="button" role="switch" aria-checked={globalShared} onClick={() => handleGlobalShareToggle(!globalShared)} className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full border transition-colors duration-200 ${globalShared ? "bg-[var(--accent)] border-[var(--accent)]" : "bg-[var(--surface)] border-[var(--border)]"}`}>
            <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform duration-200 ${globalShared ? "translate-x-5" : "translate-x-1"}`} />
          </button>
        </div>
        <div className="flex flex-col gap-2 rounded-[12px] border border-[var(--border)] bg-[var(--surface)]/30 p-3.5">
          ...
          <span className="text-[11px] leading-4 text-[var(--text-secondary)]">Clona las 8 categorías de una vez a este proyecto.</span>
          <div className="flex gap-2">
            <select value={copySource} onChange={(e) => setCopySource(e.target.value)} className="flex-1 rounded-full border border-[var(--border)] bg-[var(--bg)] px-3.5 py-2 text-[12px] text-[var(--text-primary)] outline-none focus:border-[var(--accent)]">
              <option value="">Elegí proyecto origen…</option>
              {projectList.filter((p) => p.path !== activeProject).map((p) => (
      ...
      <div className="rounded-[16px] bg-[var(--surface)]/30 p-3 border border-[var(--border)]/50">
        <p className="text-[12px] leading-5 text-[var(--text-secondary)]">
          Pegá el comando de <span className="font-mono text-[11px] bg-[var(--surface)] px-1 py-0.5 rounded border border-[var(--border)]">npx skills add … --skill …</span> que te da skills.sh, o cargá un <span className="font-mono text-[11px]">SKILL.md</span> directo.
        </p>
      </div>
