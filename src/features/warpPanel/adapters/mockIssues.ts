import type { KanbanIssue, KanbanStatus, KanbanColumn } from "../types";
import type { IssuesAdapter } from "./types";

/**
 * Mock kanban dataset — copied verbatim from
 * figma/Crear panel lateral interactivo/src/data/kanban.ts.
...
/* ─── Mock adapter (module-level mutable copy, no fetch/timers) ────────── */
...
function snapshot(store: KanbanIssue[]): KanbanIssue[] {
  return store.map(cloneIssue);
}

let store: KanbanIssue[] = snapshot(KANBAN_ISSUES);

/** Test helper: restore the module copy to the verbatim Figma seed. */
export function resetMockIssuesStore(): void {
  store = snapshot(KANBAN_ISSUES);
}

export class MockIssuesAdapter implements IssuesAdapter {
  ...
  setIssueStatus(id: number, status: KanbanStatus): KanbanIssue[] {
    store = store.map((issue) =>
      issue.id === id ? { ...cloneIssue(issue), status } : issue,
    );
    return snapshot(store);
  }
}

export const mockIssuesAdapter: IssuesAdapter = new MockIssuesAdapter();
