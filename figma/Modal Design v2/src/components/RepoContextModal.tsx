...
export default function RepoContextModal({
  ...
  const renderDetail = () => {
    ...
    const jsonFileName = selectedBrief.path.split("/").pop() || "brief.json";
      ...
      <div className="space-y-3.5">
        ...
        <div className="space-y-2.5 p-3.5 sm:p-4 rounded-lg bg-[var(--bg)] border border-[var(--border)] text-xs sm:text-sm">
          <div className="flex items-center justify-between border-b border-[var(--border)] pb-2">
            <span className="font-mono text-[11px] text-[var(--text-muted)]">{selectedBrief.timestamp}</span>
            <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-[var(--text-primary)] leading-relaxed mt-0.5 font-medium">{selectedBrief.brief.resumen_proyecto}</p>
          </div>
          ...
          {selectedBrief.brief.propuesta_de_valor && (
            <div>
              ...
              <p className="text-xs sm:text-sm text-[var(--text-secondary)] mt-0.5">{selectedBrief.brief.propuesta_de_valor}</p>
            </div>
          )}
        </div>

        {/* Collapsible JSON Viewer (starts closed by default) */}
            ...
            <div className="p-3 border-t border-[var(--border)] bg-[var(--surface)] space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() => handleCopy(JSON.stringify(selectedBrief.brief, null, 2))}
                >
                  {copied ? "¡Copiado!" : "Copiar JSON"}
                </button>
              </div>
              <JsonCodeBlock data={selectedBrief.brief} maxHeight="320px" />
            </div>
          )}
        </div>
