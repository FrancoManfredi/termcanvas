# Spec: GitHub Issues Canvas

## Context

Users visualize open GitHub issues as interactive cards on the TermCanvas infinite canvas, sourced from the focused worktree's repository. Issues are fetched via the `gh` CLI, displayed as `issue` nodes in a grid layout, and persisted across sessions through the `SceneDocument` mechanism.
