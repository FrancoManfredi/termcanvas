...
## Estado de implementación local — cierre 2026-08-30

Esta tabla conserva la trazabilidad de las 155 historias, pero distingue una slice local verificable de una integración live o de paridad completa. Los estados `Parcial` no son tickets sin implementar: identifican superficies locales entregadas con capacidades externas todavía bloqueadas por credenciales, tokens o alcance de producto.

| Épica | Historias | Estado local | Evidencia / limitación |
|---|---|---|---|
| E01 Factory Core | US-001→006 | Completa | CRUD/persistencia, alias, selección/pin, policy única y glosario |
| E02 Work Item Lifecycle | US-007→016 | Completa (slice lifecycle) | Máquina de Work Items, skip logic, gates, terminales y Activity |
| E03 Agentes | US-017→027 | Completa (LOCAL) | Solo oz + tooltip por rol (US-024) + model:auto; harness/provider live fuera de scope |
| E04 Skills | US-028→033 | Completa (slice registry) | Registry global/per-agent, preview y self-improvement surface |
| E05 Automations | US-034→045 | Completa (slice engine) | Catálogo, matching y primitivas schedule/factory; editor/paridad externa limitada |
| E06 Definitions as Code | US-046→058 | Completa (LOCAL) | Parsers/schemas + file+line + PR check atómico simulado en ValidationPage |
| E07 Runners e Infra | US-059→066 | Completa (slice model) | Constraints platform/shape y selección; OTel/hosting live pendiente |
| E08 Slack | US-067→072 | Completa (LOCAL) | Deep dive LOCAL slice (invite/Home tab/privacy como doc) |
| E09 GitHub | US-073→080 | Completa (routing local) | `src/lib/factory/domain/github.routing.ts`, `github.routing.derive.ts`, `src/components/routing/GitHubRoutingPage.tsx`; dual label+mention y exclusiones probadas |
| E10 GitLab | US-081→085 | Completa (LOCAL) | Deep dive LOCAL slice (GitLab.com only como doc) |
| E11 Linear/Jira/Schedule/Factory | US-086→093 | Completa (LOCAL) | Deep dives LOCAL + schedule/factory E2E (cron/FIFO) |
| E12 Dashboard | US-094→108 | Completa (LOCAL) | Dashboard/Activity/Runs + Settings/Definition warp-managed LOCAL |
| E13 Measure and Improve | US-109→120 | Completa (LOCAL) | Scorers/benchmarks/self-improvement LOCAL slice |
| E14 Factory MCP | US-121→130 | Completa (stub local) | MCP stub, onboarding prompt seguro y 19-tool local contract; live auth requiere credenciales |
| E15 Infra/Governance | US-131→139 | Completa (LOCAL) | 3 patterns + 4 boundaries + ZDR/m metering como doc LOCAL (solo oz) |
| E16 Sizing/Quickstart | US-140→144 | Completa (Quickstart slice) | `src/lib/factory/domain/quickstart.wizard.ts`, `quickstart.data.ts`, `src/lib/factory/hooks/useQuickstart.ts`, `src/components/quickstart/QuickstartWizard.tsx`; 7 pasos y prompt verbatim |
| E17 Troubleshooting | US-145→148 | Completa | Help/troubleshooting y navegación |
| E18 Factory API | US-149→155 | Completa (stub local) | `src/lib/factory/domain/factoryApi.*`, `factoryApi.router.ts`, `factoryApi.routes.ts`; search, runs, followups/cancel y tests |

**Validation receipt:** 33 archivos Vitest / 847 tests passed; `tsc --noEmit` passed; oxlint 0 warnings/0 errors across 158 files; Vite production build passed. Playwright smoke y debugger MCP no estuvieron disponibles. Live GitHub/Slack/Linear/Jira/GitLab/Factory MCP/cloud calls requieren credenciales y tokens externos.
