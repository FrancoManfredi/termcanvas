...
**Implementación local:** `src/components/quickstart/QuickstartWizard.tsx` + `src/lib/factory/domain/quickstart.wizard.ts` + `src/lib/factory/hooks/useQuickstart.ts`. La navegación expone `Quickstart`; el flujo crea la factory y registra el primer Work Item en el store local. La alternativa Factory MCP no inventa credenciales: muestra la ruta externa y el prompt de onboarding.
...
- [x] **Docs como spec** — este documento es la base. Actualizado a Last updated 2026-08-28 (Warp) + 2026-08-29 (TermCanvas). Schemas unauthenticated para validación.
- [x] **`web/` scaffold** — Vite + React + TS + Tailwind v4, `src/{components,lib}`. `pnpm --filter web dev` (port 5174). Build verificado. Sidebar pulido con `better-ui` + `motion`.
- [x] **Factory as Code parser** — zod `factory.yaml` v1alpha1 (case-sensitive), `factory.yaml` + `agents/<name>/agent.md` (exactly one FOREMAN) + `automations/<name>/automation.md` (AND/OR + `in`/`not_in`, cron UTC, `factory: work_item_stage_changed`) + `runners/*.yaml` (32/64 limit, `mac.version` quoted) + `scorers/*.md` (≥1 label ≥ passing y ≥1 debajo, default samplingRate 25). JSON Schema publicado como referencia. **847 tests** (33 archivos Vitest).
- [x] **GitHub mention routing dual-requirement (local)** — `factory:<alias>` + `@warp-factory` en new content, ignora edits/code blocks/bots, custom handles y simulator/trace. Ver §9 y `src/lib/factory/domain/github.routing.ts`. Live GitHub App/CI checks permanecen fuera de la slice local.
- [x] **Factory API contract (local stub)** — `GET /api/v1/factory`, `GET /api/v1/factory/{uid}`, `POST /runs`, Agent API lookup/followups/cancel y standalone run; búsqueda case-insensitive y `ticket_ref` validado. Ver §19 y `src/lib/factory/domain/factoryApi.*`.
- [x] **Automations engine v1** — triggers mock (GitHub 20 events, Slack 7, Linear 6, Jira 1, GitLab 2, schedule, factory) con matching AND/OR y `not_in` + `schedule` UTC (`@daily/@every 1h`). Filters don't control access; Single event → N automations → N runs. Simulador + filtro preview en `AutomationsPage`. **25 tests**.

**P1 — Importante (Dashboard, Measure, Runners)**

- [x] **Work Item lifecycle** — estados `Triage → Planning → Building → Reviewing → Complete/Cancelled`, foreman routing + stage skipping (`decideSkipTriage/Planning`), human approval gate antes de Building si pasó por Planning. Detail pane con `View agent`, `Event history`, `Stop task` sin confirmación. **81 tests** (`workItem.machine` + `workItem.store`).
- [x] **Factory Dashboard v1 — LOCAL slice** — Activity (kanban por stage, default `Created by = you` + 4 active stages) + Runs (team vs factory, timeline + cost + sub-agents) + Dashboard (metrics con disclaimers) **LOCAL:** `SettingsPage`/`FactoryDefinitionPage` fijados a `mode="warp-managed"` local, sin GitHub-backed read-only. — Activity (kanban por stage, default `Created by = you` + 4 active stages, Stage/CreatedBy/text search) **✓**, Runs (team vs factory, timeline + cost + sub-agents tab, `New` → foreman, `View session` steering sin chat input) **✓**, Dashboard (metrics: Total runs, PRs merged > opened posible, Autonomy = % sin human push, Cycle time medianas independientes, Cost per PR mediana split + S/M/L/XL, Most expensive PRs, Scorer cards, 3 newest Self-improvement) **✓**. **Pendiente:** Factory definition viewer (Warp-managed edit+validate+commit / GitHub read-only + link / Live-managed sin files) y Settings completo (Identity/alias, Repos, `credentialStrategy`, Analysis model, Deletion).
- [x] **Scorers v1** — CRUD + judge stub, manual scoring, `samplingRate 0` = stop auto, invariantes labels/passing. **Pagina `Scorers` + `scorer.derive` 30 tests.**
- [x] **Benchmarks v1** — 1 agent, N tasks+configs+repetitions, Scorers + **Correctness built-in**, no ganador elegido por Warp, `create task from run`. Ver `src/lib/factory/domain/benchmark.derive.ts` (`validateBenchmarkDefinition`, `deriveConfigResult`, `deriveBenchmark`, `createTaskFromRun`) + `benchmark.derive.test.ts` (278 LOC) + `BenchmarksPage.tsx`. **Hecho — checklist §17 corregido (estaba marcado pendiente en commit `c1fc80c1`).**
- [x] **Runners model** — CRUD + `platform` + `instanceShape` juntos, `mac.version` 14/15/26/27 default 26 quoted, `dockerImage` required, max 32/64, per-agent override, Environment vs Runner vs Host tabla. **52 tests**.
- [x] **Skills registry v1** — listar `skills/` globales y `agents/<name>/skills/`, preview `SKILL.md`, factory-wide vs per-agent, self-improvement puede editar. **23 tests**.
- [x] **Factory MCP stub (local) — 19 tools** — `src/lib/factory/mcp/mcp.stub.ts` (447 LOC, `MCP_TOOL_NAMES` 19 tools, `FactoryMcpStub`, `createMcpStub`, bearer headless, worktree guidance `start_working`/`message_foreman`/`list_notification_routes`) + `mcp.stub.test.ts` (392 LOC) + `McpStubPage.tsx` (167 LOC) cableado como nav `MCP tools`. Preparado para `https://app.warp.dev/api/v1/mcp/factory` real; la conexión live sigue fuera de alcance. **Hecho — checklist §17 corregido.**
- [x] **Secrets / Integrations / Runners (Settings slice)** — `SecretsPage` (scoping `factory-wide` vs `per-agent` reemplaza no agrega, `mcpServers: warpId`), `IntegrationsPage` (tabla 9 providers §9, `linear+jira` mutual exclusión), `RunnersPage` en Settings. **56 tests** (`secrets.derive` + `integrations.derive`).

**P2 — Pulido / Post-v1**

- [x] **GitLab deep dive — LOCAL slice** — GitLab.com only, Premium/Ultimate, manager token 1 year, bot naming `acme-support-warp-01k2x3y4z5` Developer role, branch `factory/<slug>` draft, `bot_mentioned` solo `repos` filter, no definition-hosting yet. **LOCAL:** `GitLabPage.tsx` + `integrations.deep.ts` GITLAB_* como doc estática, sin live OAuth. Ver §9. — GitLab.com only, Premium/Ultimate, manager token 1 year, bot naming `acme-support-warp-01k2x3y4z5` Developer role, branch `factory/<slug>` draft, `bot_mentioned` solo `repos` filter, no definition-hosting yet.
- [x] **Slack/Linear/Jira deep dives — LOCAL slice** — Slack invite + Home tab + linked account + privacy + reaction intake; Linear linked account + `agent_session_created` solo editable en files + loop caution; Jira Cloud+Rovo + keywords case-insensitive + offered to every automation. **LOCAL:** `SlackPage.tsx`/`LinearPage.tsx`/`JiraPage.tsx` + `IntegrationsDeepDivesPage.tsx` con 4 tabs, 0 creds externas. Ver §9. — Slack invite + Home tab + linked account + privacy + reaction intake; Linear linked account + `agent_session_created` solo editable en files + loop caution; Jira Cloud+Rovo + keywords case-insensitive + offered to every automation.
- [x] **Infra controls — LOCAL slice** — 3 deployment patterns (CLI-only / Warp-hosted / Self-hosted con Docker/K8s/Direct + OTel), 3 choices independientes (execution/inference/storage) Enterprise only, 4 credential boundaries (redaction backstop), ZDR, governance sin factory-specific role, metering credits, 7-step deployment checklist. Ver §13. **LOCAL:** `InfraPage.tsx` + `infra.derive.ts` con `Solo oz — sin harness externo` y tablas Environment vs Runner vs Host ya en `RunnersPage`. — 3 deployment patterns (CLI-only / Warp-hosted / Self-hosted con Docker/K8s/Direct + OTel), 3 choices independientes (execution/inference/storage) Enterprise only, 4 credential boundaries (redaction backstop), ZDR, governance sin factory-specific role, metering credits, 7-step deployment checklist. Ver §13. **Parcial: tablas Environment vs Runner vs Host ya en `RunnersPage`.**
- [x] **Troubleshooting page** — §18 está disponible en TermCanvas Help con setup, work no arranca por source, stuck/waiting on human, Stop task inmediato y causa de dos runs.
- [x] **Validación schemas + examples — LOCAL slice** — linkear `warp-factory-examples` 00/01/02/03/04/06/07 como fixtures para parser. **LOCAL:** `ValidationPage.tsx` con PR check `warp/factory-config` simulación atómica + file+line (`factory.yaml:12 — alias`) + schemas unauthenticated `https://app.warp.dev/api/v1/factory-files/schemas/v1alpha1` + `SAMPLE_*` 01/02/03/04 ya como fixtures. — linkear `warp-factory-examples` 00/01/02/03/04/06/07 como fixtures para parser. **Parcial: 01/02/03/04 ya como `SAMPLE_*`.**

**Fuera de v1 (postergar):** managed self-hosted workers reales, BYO inference/storage reales, metering real, self-improvement automático (benchmarks + self-improvement PRs reales), harnesses third-party reales, fan-out sharding.

**Próximos 2 cortes (Fase A para cerrar v1):**

- **A1: Factory definition viewer + Settings completo** — `FactoryDefinitionPage` (viewer file+line, Warp-managed editable vs GitHub read-only + link) + `SettingsPage` (Identity/alias≤60, Repos, `credentialStrategy` EXECUTOR/CREATOR, Analysis model, Deletion).
- **A2: Factory MCP stub 19 tools + Benchmarks mínimo + Self-improvement stub** — `mcp.stub.ts` (`list_tasks/search_task/get_task` con `start_working=true` worktree guidance, `send_task`, `message_foreman`) + `BenchmarksPage` (1 agent, N tasks+configs+repetitions, Correctness built-in) + `SelfImprovementPage` (3 PRs mock).

Ver `WarpFactories-UserStories.md` E12 US-095/US-108, E13 US-109+, E14 US-121+ para trazabilidad.
...
*Última actualización: 2026-08-30 — Basado en docs.warp.dev/factories (Early Access, Last updated 2026-08-28) + platform/* + warp-factory-examples. Fase 1 cerrada: `web/` integrado con 847 tests (33 archivos) — validado `tsc --noEmit` 0 errores, `oxlint` 0 warnings, `vite build` verde. Corrección §17: Factory MCP 19 tools (`mcp.stub.ts` 447 LOC + 392 LOC tests) y Benchmarks v1 (`benchmark.derive.ts` + `BenchmarksPage` + 278 LOC tests) ya estaban hechos y ahora figuran correctamente.*

*Próximo paso sugerido (Fase A para cerrar v1): A1 Factory definition viewer + Settings completo (Identity/alias, Repos, credentialStrategy, Analysis model, Deletion) y A2 Factory MCP stub 19 tools + Benchmarks mínimo + Self-improvement stub. Luego Fase B pulido (Troubleshooting page, GitLab/Slack/Linear/Jira deep dives, validación schemas + examples). Levantar `web` con `pnpm --filter web dev` (port 5174) y verificar Sidebar linkeado (Dashboard/Activity/Agents/Automations/Runs/Runners/Scorers/Skills/Secrets/Integrations/MCPs).*
