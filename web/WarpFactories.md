...
> **Estado al 2026-08-29 — commit c1fc80c1 (501 tests, `web/` integrado): Fase 1 cerrada, Fase 2 en curso.**
...
- [x] **Factory as Code parser** — zod `factory.yaml` v1alpha1 (case-sensitive), `factory.yaml` + `agents/<name>/agent.md` (exactly one FOREMAN) + `automations/<name>/automation.md` (AND/OR + `in`/`not_in`, cron UTC, `factory: work_item_stage_changed`) + `runners/*.yaml` (32/64 limit, `mac.version` quoted) + `scorers/*.md` (≥1 label ≥ passing y ≥1 debajo, default samplingRate 25). JSON Schema publicado como referencia. **501 tests**.
- [ ] **GitHub mention routing dual-requirement** — `factory:<alias>` label (auto-creada, auto-borrada) + `@warp-factory` mention, solo new content, ignora edits/code blocks/bots, dedup continue same work item, `warp/factory-config` check atómico. Ver §9 GitHub deep dive. **Pendiente: lógica dual exacta en `automation.engine` (hoy genérico).**
- [ ] **Factory API contract exacto** — `GET /factory?search=` (case-insensitive), `GET /factory/{uid}`, `POST /factory/{uid}/runs` con `prompt` required, `title` derivado, `ticket_ref` `<source>:<id>` + `ticket_url`; continuation vía Agent API (`GET /agent/runs/{runId}`, `POST /followups`, `POST /cancel`). OzSDK `client.factories.runs.create`. Ver §19. **Pendiente: stub `/api/v1/factory` mock.**
...
- [x] **Factory Dashboard v1 (parcial)** — Activity (kanban por stage, default `Created by = you` + 4 active stages, Stage/CreatedBy/text search) **✓**, Runs (team vs factory, timeline + cost + sub-agents tab, `New` → foreman, `View session` steering sin chat input) **✓**, Dashboard (metrics: Total runs, PRs merged > opened posible, Autonomy = % sin human push, Cycle time medianas independientes, Cost per PR mediana split + S/M/L/XL, Most expensive PRs, Scorer cards, 3 newest Self-improvement) **✓**. **Pendiente:** Factory definition viewer (Warp-managed edit+validate+commit / GitHub read-only + link / Live-managed sin files) y Settings completo (Identity/alias, Repos, `credentialStrategy`, Analysis model, Deletion).
...
- [ ] **Benchmarks v1** — 1 agent, N tasks+configs+repetitions, Scorers + **Correctness built-in**, no ganador elegido por Warp, `create task from run`. **Pendiente.**
...
- [x] **Factory MCP stub (parcial)** — `MCPs and apps` page team-level vs factory-level. **Pendiente:** 19 tools mock (`list_tasks`/`get_task`/`send_task`/`message_foreman` etc.) sobre mismo `WorkItemStore`, headless bearer. Preparado para `https://app.warp.dev/api/v1/mcp/factory` real.
...
- [ ] **Validación schemas + examples** — linkear `warp-factory-examples` 00/01/02/03/04/06/07 como fixtures para parser. **Parcial: 01/02/03/04 ya como `SAMPLE_*`.**
