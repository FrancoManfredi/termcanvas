...
export class FactoryMcpStub {
  ...
  constructor(store: WorkItemStore, bundle: FactoryBundle | null, opts?: { bearerToken?: string; registry?: FactoryRegistry }) {
    ...
    if (bundle) {
      ...
      // sync store knownFactories — R3: aditivo, no reemplaza las factories ya creadas.
      this.store.addKnownFactories([bundle.factory.name, ...this.factories.map((f) => f.name)]);
    } else {
      this.factories = [{ uid: "uid_payments-factory", name: "payments-factory", alias: "payments" }];
      this.store.addKnownFactories(this.factories.map((f) => f.name));
    }
  }
