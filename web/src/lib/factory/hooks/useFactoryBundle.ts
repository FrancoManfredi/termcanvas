...
// Dev-only dynamic import helper — ensures fixtures are tree-shaken in prod
// In dev/test, richer samples can be loaded via: await import("../fixtures/samples")
async function loadDevSamples(): Promise<Record<string, string> | null> {
  if (import.meta.env.DEV) {
    try {
      const mod = await import("../fixtures/samples");
      return mod as unknown as Record<string, string>;
    } catch {
      return null;
    }
  }
