...
export class WorkItemStore {
  ...
  /**
   * OCP: variante **aditiva** de `setKnownFactories`. R3 — `FactoryMcpStub` la usa desde
   * su constructor porque reemplazar el set borraría las factories creadas por el usuario.
   * No notifica: no cambia work items, solo el universo de factories admitidas.
   */
  addKnownFactories(names: readonly string[]) {
    for (const name of names) this.knownFactories.add(name);
  }

  /** Lectura del universo de factories admitidas (útil para tests y diagnóstico). */
  getKnownFactories(): readonly string[] {
    return [...this.knownFactories];
  }
