...
// Helpers — solo oz (Warp Agent) — triage/spec/implement/verify sin third-party auth
...
body`;
  ...
  it("7. harness-only agent parses and exposes harness.type/model (solo oz)", () => {
    const raw = `---
agentType: TRIAGE
harness:
  type: oz
...
body`;
    ...
    expect(r.ok).toBe(true);
    expect(r.value!.harness!.type).toBe("oz");
    expect(r.value!.harness!.model).toBe("auto");
    expect(r.value!.harness!.auth).toBeUndefined();
  });
...
describe("AgentsPage — UI contract (static GitHub-backed read-only, WarpFactories §10)", () => {
  ...
  it("24. selecting an agent shows AgentDetail with description + instructions", async () => {
     ...
     await act(async () => {
       container = render(React.createElement(AgentsPage)).container;
     });
  ...
  it("25. AgentDetail shows harness when present, model when present, runner, workerHost, secrets (redacted), mcpServers, and read-only button", async () => {
    ...
    // en modo solo-oz, todos los agentes usan model:auto, no harness; buscamos el primero con model
    const withModel = bundle.value!.agents.find((a) => !!a.model)!;
    const factory = bundle.value!.factory;
    render(React.createElement(AgentDetail, { agent: withModel, factory }));
    expect(screen.getAllByText(new RegExp(withModel.model!, "i")).length).toBeGreaterThan(0);
    expect(screen.getAllByText(/••••/).length).toBeGreaterThan(0);
    expect(screen.getAllByText(new RegExp(withModel.instructions.slice(0, 12), "i")).length).toBeGreaterThan(0);
    const btn = screen.getByRole("button", { name: new RegExp(withModel.rawPath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i") });
