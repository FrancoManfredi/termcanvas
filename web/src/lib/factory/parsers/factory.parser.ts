import { parseYamlSafe, parseYamlWithLineCounter } from "./yaml.utils";
...
export class FactoryParser implements IFactoryParser {
  parseFactoryYaml(raw: string, file = "factory.yaml"): ParseResult<FactoryDefinition> {
    ...
    if (!parsed.success) {
      // P1-01: file+line real via yaml LineCounter (ya es dep), fallback a findLineForPath si no resuelve
      const { lineCounter } = parseYamlWithLineCounter(raw);
      const issues = zodToParseIssues(parsed.error, file, raw, lineCounter);
