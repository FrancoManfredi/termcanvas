...
export function ActivityBoard() {
  ...
  const [includeTerminals] = useState(false);
  void includeTerminals;
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [collapsed, setCollapsed] = useState<Record<string, boolean>>({ Planning: true, Building: true });

  useEffect(() => {
    seedIfEmpty();
    // auto-select first Triage item to match image (93C5DB)
    const first = getWorkItemStore().list({ includeTerminals: true }).find((w) => w.stage === "Triage");
    if (first) {
  ...
  const { items, transition, cancel } = useWorkItems(filter);
  ...
  const selectedItem = useMemo(() => {
    ...
    return getWorkItemStore().getById(selectedId) ?? null;
  }, [selectedId, items]);
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-[#f8f8f8]">
      ...
      <div className="flex h-[44px] shrink-0 items-center justify-between border-b border-zinc-200 bg-white px-4">
        ...
        <div className="flex items-center gap-1">
          <button className="grid h-7 w-7 place-items-center rounded-[8px] border border-zinc-200 bg-white text-zinc-500 hover:bg-zinc-50">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-zinc-200 bg-white px-4 py-2">
        <div className="flex items-center gap-1 rounded-full border border-zinc-200 bg-white px-2 py-1 text-xs">
          ...
          {stages.length > 0 && (
            <button onClick={() => setStages([])} className="ml-1 grid h-4 w-4 place-items-center rounded-full hover:bg-zinc-100">
              ×
            </button>
          )}
        </div>
        {createdBy && (
          <div className="flex items-center gap-1 rounded-full border border-zinc-200 bg-white px-2 py-1 text-xs">
            ...
            <button onClick={() => setCreatedBy("")} className="ml-1 grid h-4 w-4 place-items-center rounded-full hover:bg-zinc-100">
              ×
            </button>
          </div>
        )}
        <button
          ...
          className="grid h-6 w-6 place-items-center rounded-full border border-dashed border-zinc-300 text-zinc-500 hover:bg-zinc-50"
        >
              ...
              return (
                <div key={stage} className="overflow-hidden rounded-[10px] border border-zinc-200 bg-white">
                  <button
                    ...
                    className={`flex w-full items-center gap-2 px-3 py-2 text-left ${cfg.bg} border-b border-zinc-200`}
                  >
                        ...
                        stageItems.map((item) => {
                          ...
                          return (
                            <button
                              ...
                              className={[
                                "flex w-full items-center gap-3 px-3 py-2.5 text-left transition-[background-color] duration-150",
