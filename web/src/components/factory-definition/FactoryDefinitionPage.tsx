...
export function FactoryDefinitionPage({ mode = "warp-managed" }: FactoryDefinitionPageProps) {
  ...
  if (mode === "live-managed") {
    return (
      <div className="flex min-h-0 flex-1 flex-col overflow-auto bg-[#f6f6f6] p-4">
  ...
  return (
    <div className="flex min-h-0 flex-1 overflow-hidden bg-[#f6f6f6]">
      {/* File list */}
        ...
        <div className="border-b border-zinc-200 px-3 py-2.5">
          ...
          {mode === "github-backed" && (
            <div className="mt-2 rounded-[8px] border border-amber-200 bg-amber-50 px-2.5 py-2">
              <p className="text-[11px] font-medium text-amber-800">managed in GitHub, edit via PR</p>
              <p className="mt-0.5 text-[11px] text-zinc-600">Dashboard read-only · link file+line al repo</p>
            </div>
          )}
          {mode === "warp-managed" && (
            <p className="mt-1.5 text-[11px] text-zinc-500">Warp-managed es editable+valida+commit atómico (estático: solo lectura)</p>
          )}
        </div>
