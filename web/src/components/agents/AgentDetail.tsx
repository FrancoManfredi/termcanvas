...
function getOptimizationHint(type: string): { label: string; detail: string } {
  switch (type) {
    case "FOREMAN":
    case "MAIN":
      return { label: "Orchestration", detail: "Instruction following, long-running conversations, enrutamiento entre agentes y handoff humano (§4)" };
    case "TRIAGE":
      return { label: "Research", detail: "Evidence gathering, trabajo con tools conectados, reproducir solo si investigación no alcanza (§4)" };
    case "SPEC":
      return { label: "Technical reasoning", detail: "Síntesis de requirements, escritura precisa de product+technical specs en draft PR (§4)" };
    case "IMPLEMENT":
      return { label: "Coding strength", detail: "Coding con harness que matchee repos/toolchain, tests+validation+evidencia visual, nunca mergea (§4)" };
    case "REVIEW":
      return { label: "Blind spots", detail: "Modelo/harness distinto al implement para no compartir blind spots, chequeo independiente (§4)" };
    case "VERIFY":
      return { label: "Verification", detail: "Custom VERIFY, no required para todo work item, disparable por automation específica (§4)" };
    default:
      return { label: "Custom", detail: "Extiende baseline GitHub/Slack/tracker, no amplía acceso (secrets/MCPs en config, no en skill) (§5)" };
  }
}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        ...
        <div className="grid gap-3">
          ...
          <div className="rounded-[8px] border border-violet-200 bg-violet-50 px-3 py-2">
            <p className="text-[10px] font-[600] tracking-[0.06em] uppercase text-violet-700">Optimizar por rol (WarpFactories.md §4 — US-024)</p>
            <p className="mt-1 text-[11px] leading-snug text-violet-800"><span className="font-[600]">{getOptimizationHint(agent.agentType).label}</span> — {getOptimizationHint(agent.agentType).detail}</p>
          </div>
