...
import { deriveDashboardMetrics } from "../../lib/factory/domain/dashboard.derive";
...
export function DashboardPage() {
  const { store } = useWorkItems({ includeTerminals: true });
  // use workItems to trigger reactivity, but derive uses store directly for DIP
  ...
  const metrics = useMemo(() => {
    ...
    return m;
  }, [store, bundleResult, store.getVersion()]);
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="mx-auto max-w-[1080px] space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <MetricCard title="Total runs" value={metrics.totalRuns.total} subtitle="Todos los agent runs (incl. evaluation/benchmark/self-improvement)">
            ...
            <MetricCard title="PRs opened" value={metrics.prsOpened} subtitle="PRs creados por factory work, contados una vez" />

            <MetricCard title="PRs merged" value={metrics.prsMerged} subtitle="De los PRs opened en el periodo, cuántos mergearon luego (puede > opened si merged viene de datasource distinto)" />

            <MetricCard title="Autonomy %" value={formatPct(metrics.autonomyPct)} subtitle="% de merged PRs sin human code push antes de mergear (opening PR cuenta como push; comments/reviews no cuentan)" />
          </div>

          {/* Row 2: cycle time + cost */}
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <MetricCard title="PR cycle time" value={formatDuration(metrics.prCycleTimeMedianMs)} subtitle="Mediana merged PRs: run kickoff → PR → first review → merge (median independientes por stage)">
              <ul className="space-y-1">
            ...
            <MetricCard title="Cost per PR" value={formatCost(metrics.costPerPrMedian)} subtitle="Mediana costo PRs opened, spliteado equitativo si 1 run → >1 PR (estimate, no billing)">
            ...
            <MetricCard title="Scorer cards" value={metrics.scorerCards.length} subtitle="Resultados de Scorers configurados">
          ...
          {/* Row 3: Most expensive + Self-improvement */}
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            ...
            <div className="rounded-[12px] border border-zinc-200 bg-white p-4">
              <div className="text-[11px] font-[600] tracking-[0.06em] uppercase text-zinc-500">Self-improvement PRs</div>
              <div className="mt-1 text-[11px] text-zinc-500">Los 3 newest Self-improvement PRs, sin importar el date range</div>
              {metrics.selfImprovementPrs.length === 0 ? (
                <p className="mt-3 text-[12px] text-zinc-400">Sin PRs</p>
              ) : (
                <ul className="mt-3 space-y-2">
