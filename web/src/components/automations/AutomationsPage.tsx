      ...
      <div className="flex h-[44px] shrink-0 items-center justify-between border-b border-zinc-200 bg-white px-4">
        <div className="flex items-center gap-1.5 text-[13px]">
          ...
          <span className="ml-2 hidden sm:inline-flex">
            <HelpLink anchor="work-not-starting" label="Ayuda: Work isn't starting — automations" />
          </span>
        </div>
        <span className="flex items-center gap-2 text-[11px] text-zinc-500">
          <span>Factory as Code · automations/*/automation.md</span>
          <HelpLink anchor="work-not-starting" label="Ayuda: Work isn't starting — automations" />
        </span>
      </div>

      <div className="flex min-h-0 min-w-0 flex-1 overflow-hidden">
        <div className="flex min-w-0 flex-1 flex-col overflow-y-auto border-r border-zinc-200 bg-panel">
          <div className="bg-white border-b border-zinc-200 p-3">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <label className="text-[11px] text-zinc-600">
                provider
                <select value={provider} onChange={(e) => handleProviderChange(e.target.value)} className="mt-1 w-full rounded-[8px] border border-zinc-200 bg-white px-2 py-1.5 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/20">
              ...
              <label className="text-[11px] text-zinc-600">
                event
                <input value={event} onChange={(e) => handleEventChange(e.target.value)} placeholder="issue_labeled" className="mt-1 w-full rounded-[8px] border border-zinc-200 bg-white px-2 py-1.5 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/20" aria-label="Event" />
              </label>
              <label className="text-[11px] text-zinc-600">
                repos (coma)
                <input value={repos} onChange={(e) => setRepos(e.target.value)} placeholder="acme/payments-service" className="mt-1 w-full rounded-[8px] border border-zinc-200 bg-white px-2 py-1.5 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/20" aria-label="Repos" />
              </label>
              <label className="text-[11px] text-zinc-600">
                labels (coma)
                <input value={labels} onChange={(e) => setLabels(e.target.value)} placeholder="factory-ready" className="mt-1 w-full rounded-[8px] border border-zinc-200 bg-white px-2 py-1.5 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/20" aria-label="Labels" />
              </label>
              <label className="text-[11px] text-zinc-600">
                branches
                <input value={branches} onChange={(e) => setBranches(e.target.value)} placeholder="main" className="mt-1 w-full rounded-[8px] border border-zinc-200 bg-white px-2 py-1.5 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/20" aria-label="Branches" />
              </label>
              <label className="text-[11px] text-zinc-600">
                base_branches
                <input value={baseBranches} onChange={(e) => setBaseBranches(e.target.value)} placeholder="main" className="mt-1 w-full rounded-[8px] border border-zinc-200 bg-white px-2 py-1.5 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/20" aria-label="Base branches" />
              </label>
            </div>
            <label className="mt-2 block text-[11px] text-zinc-600">
              schedule (para cron_fired)
              <input value={schedule} onChange={(e) => setSchedule(e.target.value)} placeholder="@daily" className="mt-1 w-full rounded-[8px] border border-zinc-200 bg-white px-2 py-1.5 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/20" aria-label="Schedule" />
            </label>
            <div className="mt-2 flex flex-wrap gap-1.5">
              <button onClick={handlePresetFactoryReady} className="rounded-full border border-zinc-200 bg-white px-2.5 py-1 text-[11px] font-medium text-zinc-700 hover:bg-zinc-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/20">preset: issue_labeled factory-ready</button>
              <button onClick={handlePresetWip} className="rounded-full border border-zinc-200 bg-white px-2.5 py-1 text-[11px] font-medium text-zinc-700 hover:bg-zinc-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/20">preset: PR wip → no match</button>
              <button onClick={handlePresetSchedule} className="rounded-full border border-zinc-200 bg-white px-2.5 py-1 text-[11px] font-medium text-zinc-700 hover:bg-zinc-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/20">preset: schedule @daily</button>
            </div>
            <div className="mt-3 rounded-[8px] border border-zinc-200 bg-zinc-50 p-2">
              ...
              {matched.length > 1 && (
                <div className="mt-2 rounded-[6px] border border-amber-200 bg-amber-50 px-2 py-1.5 text-[11px] leading-snug text-amber-800">
                  ⚠️ <span className="font-[600]">Overlapping:</span> 1 evento → {matched.length} runs (ej. Slack app_mention + message_posted en mismo channel). Angosta con <span className="font-mono">not_in</span> o remueve trigger solapado. WarpFactories.md §6 + Troubleshooting “One action → two runs”.
                </div>
              )}
            </div>
            <div className="mt-3 rounded-[8px] border border-violet-200 bg-violet-50 p-3">
              <p className="text-[11px] font-[600] tracking-[0.06em] uppercase text-violet-700">Catálogo 06-common-automations (US-045) — presets copiables</p>
              <p className="mt-1 text-[11px] text-violet-600">LOCAL slice: branches/paths/workflow conclusions/emoji reactions. Sin Warp Cloud, son ejemplos para copy-paste a `automation.md`.</p>
              <div className="mt-2 grid gap-2">
                <div className="rounded-[6px] bg-white p-2 font-mono text-[11px] leading-relaxed text-zinc-700">
                  <div className="font-[600] text-zinc-800">branches + paths</div>
                  <pre className="mt-1 whitespace-pre-wrap text-[11px]">{`triggers:
  - provider: github
    event: pull_request_opened
    filter:
      repos: [acme/api-service]
      ...
      branches: [feature/*]`}</pre>
                ...
                <div className="rounded-[6px] bg-white p-2 font-mono text-[11px] leading-relaxed text-zinc-700">
                  <div className="font-[600] text-zinc-800">workflow conclusions [failure]</div>
                  <pre className="mt-1 whitespace-pre-wrap text-[11px]">{`triggers:
  - provider: github
    ...
    filter:
      repos: [acme/api-service]
      branches: [main]
      conclusions: [failure]`}</pre>
                </div>
                <div className="rounded-[6px] bg-white p-2 font-mono text-[11px] leading-relaxed text-zinc-700">
                  <div className="font-[600] text-zinc-800">emoji reaction [ticket]</div>
                  <pre className="mt-1 whitespace-pre-wrap text-[11px]">{`triggers:
  - provider: slack
    event: reaction_added
    filter:
      channels: [factory-intake]
      emojis: [ticket]`}</pre>
                </div>
                <div className="rounded-[6px] bg-white p-2 font-mono text-[11px] leading-relaxed text-zinc-700">
                  <div className="font-[600] text-zinc-800">schedule — weekly audit</div>
                  <pre className="mt-1 whitespace-pre-wrap text-[11px]">{`triggers:
  - provider: schedule
    event: cron_fired
    schedule: {name: weekly-dependency-audit, cron: "0 9 * * 1"} # UTC`}</pre>
                </div>
              </div>
            </div>
          </div>
      ...
      {selectedAutomation && (
        <div className="fixed inset-0 z-10 flex flex-col bg-white lg:hidden">
          <div className="flex justify-end border-b border-zinc-200 p-2">
            <button onClick={() => setSelected(null)} className="rounded-full border border-zinc-200 bg-white px-3 py-1 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/20">Cerrar</button>
          </div>
