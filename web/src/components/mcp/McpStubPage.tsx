...
import { WorkItemStore } from "../../lib/factory/store/workItem.store";
import { WorkItemMachine } from "../../lib/factory/domain/workItem.machine";
...
export function McpStubPage() {
  ...
  // single store instance for this page — real WorkItemStore, headless bearer stub
  const { store, stub } = useMemo(() => {
    const s = new WorkItemStore(new WorkItemMachine(), bundle ? [bundle.factory.name] : ["payments-factory"]);
    // seed with 2 demo tasks for interactive exploration
    s.create({ factoryName: bundle?.factory.name ?? "payments-factory", title: "Fix checkout race at app/checkout.ts", description: "Goal: fix race; Context: checkout flow", source: "github_issue", createdBy: "ben" });
    s.create({ factoryName: bundle?.factory.name ?? "payments-factory", title: "Add Local development section to README", description: "Single file, run lint", source: "mcp", createdBy: "mcp-agent" });
    const st = new FactoryMcpStub(s, bundle);
    return { store: s, stub: st };
  }, [bundle]);
