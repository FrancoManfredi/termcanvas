...
## Toolchain dual (root vs web) — dos productos intencionales

Este monorepo contiene **dos toolchains intencionalmente distintos** — no es un bug, es diseño. OCP: `web/src/**` no se toca desde root (Ola 9, `8f270cfb`, 203 files, 39/902).

| Producto | Comando gate | Runner | Estado (Ola 10a `2026-08-30`) |
|---|---|---|---|
| **TermCanvas Electron (root)** | `pnpm test` → `tsx --test` (ver `package.json:12` `scripts.test`, línea 23) | Node `tsx --test` sin Vitest | `957 tests · 952 pass · 2 fail (flakes conocidos) · 3 skipped` — suites: canvas, pty, issueStore, sessionWatcher, terminal, etc. |
| **Warp Factories (web)** | `pnpm --filter web check` → `tsc --noEmit && tsc -b && vitest run --pool=threads && vite build && oxlint` (ver `web/package.json:14` `scripts.check`) | `vitest 4.1.11` 902/902 + `oxlint 1.79.0` 0 errores + `vite 8.2.2` 2319 módulos | Verde |

- **Workspace verificado:** `pnpm-workspace.yaml:7` intacto — `packages: [agent, browse, eval, hydra, website, web]` (incluye `web`).
- **Project References (web):** `web/tsconfig.json` → `tsconfig.app.json` (DOM, bundler, react-jsx) + `tsconfig.node.json` (nodenext, incluye `vite.config.ts` y `vitest.config.ts`).

### Zod drift — pinneado no necesario

- `root` `zod ^4.3.6` (`package.json:83`) vs `web` `zod 4.5.4` (`web/package.json:28`) — drift menor `4.3 → 4.5`.
- `shared/` **no** comparte runtime Zod: ningún `shared/*.ts` importa `zod` (solo modelos, `mcp`, `telemetry`, etc.).
- `web` usa `zod` solo en `web/src/lib/factory/schemas/*` + `web/src/lib/factory/parsers/*` (validación YAML de factories).
- No hay código compartido que instancie `z.ZodError` entre root y web, por lo que **no se requiere pinneado** en esta ola. Si en el futuro `shared/` expusiera schemas Zod, alinear a `^4.5.4` y `pnpm install`.

> **Evidencia gates (capturada Ola 10a, 2026-08-30):**
> - `pnpm test 2>&1 | tail` (root): `ℹ tests 957 · pass 952 · fail 2 · skipped 3 · duration_ms 77431`
>   - `✖ git-watcher` timeout FS (flaky) · `✖ pty-launch` shims PATH en Windows (espera 2 entradas, ve 1) — no tocan `web`.
> - `pnpm --filter web check 2>&1 | tail` (web): `Test Files 39 passed · Tests 902 passed · vite built 2319 modules · oxlint 0`
